  ...
  return (
    <div className="pt-4 my-2">
      <label className="relative">
        <span className="absolute -top-0.5 right-full left-2 opacity-80 transition-all duration-300">{props.label}</span>
        ...
        <input type={props.type} name={props.name} id={props.id} onChange={props.onChange} className={`w-full bg-transparent focus:ring-0 border-0 border-b text-black focus:border-black ${isValue && 'border-black'} h-10 px-2 group-focus:bg-black`} onLoad={inputLoad} onFocus={inputForm} onBlur={outInput}/>
      </label> 
    </div>
  )
}
